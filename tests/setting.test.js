...
const setting = require('../setting'); // assuming this is the file containing the Express app
...
describe('POST /setting', () => {
  it('updates user information and redirects to the settings page', async () => {
    ...
    await request(setting)
      .post('/register') // assuming this route creates a new user
      .send(mockUser);
    // const user = new User(mockUser);
    // await user.save();

    // send a mock POST request to update the user information
    const response = await request(setting)
      .post('/setting')
      .send({
        id: '123',
        name: 'John Doe',
